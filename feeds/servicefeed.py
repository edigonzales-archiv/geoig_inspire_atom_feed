...
try:
    ...
    sql = """SELECT array_agg(uri) as uri, identifier, array_agg(format_txt) as format_txt, array_agg(srs_txt) as srs_txt, array_agg(srs_epsg) as srs_epsg, array_agg(modified) as modified, array_agg(format_mime) as format, array_agg(bbox) as bbox, array_agg(title) as title, array_agg(abstract) as abstract
FROM
(
SELECT a.*, c.uri, c.format_mime as format_mime, c.format_txt, c.srs_txt, c.srs_epsg, round(y_min::numeric, 8) || ' ' || round(x_min::numeric, 8) || ' ' || round(y_max::numeric, 8) || ' ' || round(x_max::numeric, 8) as bbox
FROM metadb_ng.metadb as a,
(
 SELECT metadb_id_child as id
 FROM metadb_ng.metadb_relation
 WHERE metadb_id_parent = '93eb36e5-5f16-4d98-b704-f6e087f647b8'
) as b,
metadb_ng.online_dataset as c
